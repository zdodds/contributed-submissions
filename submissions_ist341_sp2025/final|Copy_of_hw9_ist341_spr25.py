# good_name_for_this_project.py

import openai

# API key will go here (replace 'your-api-key-here' with your real API key)
...
def get_concert_outfit(vibe_description):
    prompt = f"""
    You are an expert fashion stylist specializing in concert outfits for.
    Based on the following description of a concert's vibe, recommend a stylish outfit:
    "{vibe_description}"
    ...
    response = openai.ChatCompletion.create(
        model="gpt-4o",   # or "gpt-3.5-turbo" if using lower cost
...
if __name__ == "__main__":
    vibe = "Outdoor festival, indie rock, cloudy weather"
    result = get_concert_outfit(vibe)
...
os.environ["REPLICATE_API_TOKEN"] = "r8_9rvMrbQ1BYJuEFktQXGytxRyyWMNR7A1GAhg3"  # your token here


def get_image_caption(image_path):
    output = replicate.run(
        "salesforce/blip-2:5b329ac94f44f57b1518ab6c3c0df0fcbe1f650d738ad6ed6ff6f014d3f6a4d6",
...
# ✅ New client object (v1+)
client = openai.OpenAI(api_key="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")  # replace with your key

def get_concert_outfit(vibe_description):
    prompt = f"""
    You are a fashion stylist. Based on the following concert vibe, suggest an outfit:
    "{vibe_description}"
...
# ✅ Gradio app
demo = gr.Interface(
    fn=get_concert_outfit,
    inputs=gr.Textbox(lines=2, label="Concert Vibe", placeholder="e.g. Outdoor EDM concert, warm weather"),
    outputs=gr.Textbox(label="Outfit Recommendation"),
    title="🎤 Concert FitFinder AI",
    description="Enter the concert vibe and get a stylish outfit from AI!",
