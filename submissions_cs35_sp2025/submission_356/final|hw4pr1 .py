...
# 1. lineplot, timeseries plot with error bands
# url: https://seaborn.pydata.org/examples/errorband_lineplots.html
...
# 2. displot, facetting histograms by subsets of data
# url: https://seaborn.pydata.org/examples/faceted_histogram.html
...
# 3. boxplot, grouped boxplots
# url: https://seaborn.pydata.org/examples/grouped_boxplot.html
...
births2 = births_orig.drop('from http://chmullig.com/2012/06/births-by-day-of-year/', axis=COLUMN)

sns.set_theme(style="whitegrid")

# Plot the responses for different events and regions
sns.barplot(x="month",y="births", hue="above/below median", data=births2)
...
# Plot the responses for different events and regions
sns.barplot(x="pclass",y="survived", hue="sex", data=titanic2)
...
h1 = housing_orig.drop('how many datasets have journal papers? This one does!  http://jse.amstat.org/v19n3/decock.pdf', axis=1)

plt.figure(figsize=(15, 8))

# Create a box plot with swarm overlay
sns.boxplot(data=h1, x='Overall Qual', y='SalePrice', color='lightblue')
sns.swarmplot(data=h1, x='Overall Qual', y='SalePrice', size=4, alpha=0.5, color='navy')

# Customize the plot
plt.title('House Prices by Overall Quality Rating', fontsize=14)
plt.xlabel('Overall Quality (1-10 Scale)', fontsize=12)
...
# We import import our dataframe from a csv, using pandas
pokemon_orig = pd.read_csv('./Pokemon.csv',encoding='latin1')

ROW = 0
COLUMN = 1

pokemon2 = pokemon_orig.drop('#', axis=COLUMN)

sns.set_theme(style="whitegrid")

# Plot the responses for different events and regions
sns.barplot(x="Legendary",y="Total", hue="Type 1", data=pokemon2)
...
house_df = house_df.drop('how many datasets have journal papers? This one does!  http://jse.amstat.org/v19n3/decock.pdf', axis=1)
# Create a Linear Regression model
from sklearn.linear_model import LinearRegression

# Create and fit the model
model = LinearRegression()
X = house_df[['Lot Area']].values  # Input feature as 2D array
y = house_df['SalePrice'].values    # Target variable as 1D array
model.fit(X, y)
...
# Plot the results
plt.figure(figsize=(8, 6))
plt.scatter(X, y, color='blue', label='Lot Area')
plt.plot(X, y_pred, color='red', label='Linear regression')
plt.xlabel('Lot Area')
...
def predict_sale_price(Lot_Area):
        # Reshape input to 2D array with 1 sample, 1 feature 
    X_new = [[Lot_Area]]
...
# Test the function with a sample value
test_lot_area = 8500
predicted_price = predict_sale_price(test_lot_area)
print(f"For a lot area of {test_lot_area}, predicted sale price is {predicted_price:.2f}")
