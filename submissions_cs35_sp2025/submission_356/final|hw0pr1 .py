...
print(f"I am originally from {hometown}, now a {year} at {college}.")
...
def mystery1( s ):
    """ mystery function #1 """
    ''' return the number of i or Is in input string s'''
    result = 0
...
def times42( s ):
    ''' returns input string s 42 times on separate lines
    '''
...
def alien( N ):
    ''' returns string "alien" with N "i"s
    '''
    eyes = "i" * N
    print(f"al{eyes}en")
...
def count_digits( s ):
    ''' returns number of digits in input string s '''
    result = 0
...
def just_text( s ):
    ''' returns only the letters now in lower-case
    '''
    text = []
...
def tennis():
    ''' simulates a simplified tennis match and returns score for every point randomly played'''
    ### tennis matches don't necessarily declare 50 or whatever abstract number to close the game
    ### deuces also not incorporated

    print(f"Ready!")

    points = [0,15,30,40,50] # define what the points of the match is 
    player1 = 0 # player 1 starts at 0
    player2 = 0 # player 2 starts at 0
    score = (player1,player2) # first number in a score is the player who serves, for simplicity's sake player1
    
    print(f"Initial score: {score}-- Love all")

    while True:
        if random.choice([True,False]):
            player1 = points[points.index(player1) + 1] if player1 !=50 else "GAME"
            print(f"Player 1 scores. {player1,player2}") # always declare the score after every point
            if player1 == 50:
                print(f"GAME! Player 1 wins.") 
                break
        else:
            player2 = points[points.index(player2) + 1] if player2 !=50 else "GAME"
            print(f"Player 2 scores. {player1,player2}")
            if player2 == 50:
...
# team with CS35_Participant_2 lim

def hello_patrick(text):
    """
    Converts string to randomly alternate uppercase/lowercase letters).
    """
    return ''.join(random.choice([char.upper(), char.lower()]) for char in text)

test1 = "Python is super fun!"
test2 = "I love McNuggets!"
test3 = "Let's be friends!"

print(hello_patrick(test1))
print(hello_patrick(test2))
print(hello_patrick(test3))
