...
# first 0:3 refers to the rows and 0:2 refers to the numbers in the lanes
...
zsp = zillow.loc[zillow['SalePrice'] < 300000]
zys = zsp.loc[zsp['Yr Sold'] == 2010]
zla = zys.loc[zys['Lot Area'] > 10000]
znumb = zla.loc[zla['Bedroom AbvGr'] == 3]
zms = znumb.loc[znumb['Mo Sold'] == 6]
zover = zms.loc[zms['Overall Qual'] == 9]
...
iris_sortedset = iris.sort_values(by=['sepallen'])



iris_sortedset['x_value'] = np.arange(0,len(iris_sortedset))   # this is like an "index," but it's just values
iris_sortedset
...
# Plot the responses for different events and regions
sns.lineplot(x="x_value",y="sepallen", hue="irisname", data=iris_sortedset)
...
birth_sorted = birth.sort_values(by=['month'])


birth_sorted['births']= np.arange(0, len(birth_sorted))
birth_sorted
...
# Plot the responses for different events and regions
sns.lineplot(x="births",y="month", hue="above/below median", data=birth_sorted)


ic_orig = pd.read_csv('./iris_cleaned.csv')
ic_orig
...
iris_sorted0 = ic_orig.sort_values(by=['sepalwid'])


iris_sorted0['x_value'] = np.arange(0,len(iris_sorted0))   # this is like an "index," but it's just values
iris_sorted0
...
# Plot the responses for different events and regions
sns.lineplot(x="sepallen",y="sepalwid", hue="irisname", data=iris_sorted0)


# Load the iris data from iris_cleaned.csv
births_df = pd.read_csv('births.csv')   # this is a dataframe
...
X = births_df[['month']].values  # Input feature as 2D array
y = births_df['day'].values    # Target variable as 1D array
model.fit(X, y)
...
plt.xlabel('Month')
plt.ylabel('day')
plt.title('Linear Regression: Month vs Day')
plt.legend()
plt.show()
...
test_month = 3
predicted_day = predict_sepal_width(test_month)
print(f"For the {test_month} of the year, predicted day is {predicted_day:.2f}")
...
# Create scatter plot of heights data
plt.figure(figsize=(10,6))
sns.scatterplot(data= sephora, x='loves_count', y='price_usd', alpha=0.5)

# Fit linear regression
from sklearn.linear_model import LinearRegression
X = sephora[['loves_count']].values
y = sephora['price_usd'].values
reg = LinearRegression().fit(X, y)

# Plot regression line
plt.plot(X, reg.predict(X), color='red', linewidth=2)

plt.title('Love Count vs Price with Linear Regression')
