...
FRONT = len('<td class="remove-img-span"><a href="/Damage" title="Damage"><span class="color-red">')
url = "https://wiki.hypixel.net/Aspect_Of_The_Dragons"
result = requests.get(url)
print(f"{result = }")
end = 0
AOTD = result.text     
...
while True:
    start = AOTD.find('<td class="remove-img-span"><a href="/Damage" title="Damage"><span class="color-red">', end)
    if start == -1: break     # stop if we're done!
    end = AOTD.find('</span></a>', start)  # find the correct ending!
    
    AOTD_damage = AOTD[ start+FRONT:end ]
    print(f"{AOTD_damage = }")
...
re.findall(r"Harv",  "Harvey Mudd, not Harvard Med!")       # findall works ... but it's not very informative here ...
...
re.sub(r"42*", "42", "Favorite #'s:  4 42 422 4222 42222 422222")       # try + {2}  {1,3}   (42)
...
Currently under construction, under deadline, and undeterred by the strong SoCal sun... 
...
def handle_list_blue(contents):
    ...
    for line in OldLines:
        new_line1 = re.sub(r'(<li>)', r'\1<span style="background-color:green;">', line)
