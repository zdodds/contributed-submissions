...
if not filtered_df.empty:
    print("Order Number(s):", (filtered_df.index + 1).tolist())
    display(filtered_df)
...
births_by_month = births_df.groupby("month")["births"].sum()

sns.set_theme(style="whitegrid")


plt.figure(figsize=(10, 5))
sns.lineplot(x=births_by_month.index, y=births_by_month.values, marker="o")
plt.title("Number of Births by Month")
plt.xlabel("Month")
...
plt.figure(figsize=(10, 5))
sns.scatterplot(x=iris_df["petallen"], y=iris_df["petalwid"], hue=iris_df["irisname"])
...
plt.title("Survival Count by Passenger Class (Titanic Dataset)")
...
# you will want at least three additional visualizations here... 
...
digit_values = digits_df.iloc[0, :-2].values.reshape(8, 8) 
plt.figure(figsize=(6, 6))
sns.heatmap(digit_values, cmap="coolwarm", linewidths=0.5, annot=True, cbar=True)
plt.title(f"Heatmap of Digit: {digits_df.iloc[0]['actual_digit']}")
plt.show()


plt.figure(figsize=(10, 5))
sns.violinplot(x=titanic_df["survived"], y=titanic_df["age"])
plt.title("Age Distribution by Survival Status (Titanic Dataset)")
...
sns.lineplot(x=X_test["age"], y=y_pred, color="red", label="Regression Line")
plt.xlabel("Age")
plt.ylabel("Fare Price")
plt.title(f"Linear Regression: Age vs. Fare (Titanic Dataset)\nR² = {r2:.4f}")
plt.legend()
plt.show()
