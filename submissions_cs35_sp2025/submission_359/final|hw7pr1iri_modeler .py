...
listOfX_all = []
for i in range(1,4):
    listOfX_all += [np.concatenate( (A[:,0:i], A[:,(i+1):]), axis=1)]
...
for i in range(len(listOfY_all)):
    print(f"y_all to predict {COLUMNS[i+1]} is \n {listOfY_all[i]}")
    print() 
    print(f"X_all to predict {COLUMNS[i+1]} (the features, first few rows) is \n {listOfX_all[i][:5,:]}")
...
# we scramble both X and y, necessarily with the same permutation
for i in range(len(listOfX_all)):
    listOfX_all[i] = listOfX_all[i]
#X_all = X_all[indices]              # we apply the _same_ permutation to each!
#y_all = y_all[indices]              # again...
for i in range(len(listOfY_all)):
    listOfY_all[i] = listOfY_all[i]
#print("target values to predict: \n",y_all)

for i in range(len(listOfX_all)):
    print("target values to predict for", COLUMNS[i+1], ":", listOfY_all[i])
    print("X_all to predict", COLUMNS[i+1], "features (a few)\n", listOfX_all[i][:5,:])
...
for i in range(len(listOfX_all)):
    splitList = train_test_split(listOfX_all[i], listOfY_all[i], test_size=0.2)
    listOfX_train += [splitList[0]]
    listOfX_test += [splitList[1]]
    listOfY_train += [splitList[2]]
    listOfY_test += [splitList[3]]
    #listOfX_train, listOfX_test[i], listOfy_train[i], listOfy_test[i] = train_test_split(listOfX_all[i], y_all, test_size=0.2)

for i in range(len(listOfX_all)):
    print(f"training with {len(listOfY_train[i])} rows for {COLUMNS[i+1]};  testing with {len(listOfY_test[i])} rows\n" )

    print(f"Held-out data... (testing data: {len(listOfY_test[i])})")
    print(f"y_test: {listOfY_test[i]}\n")
    print(f"X_test (few rows): {listOfX_test[i][0:5,:]}")  # 5 rows
    print()
    print(f"Data used for modeling... (training data: {len(listOfY_train[i])})")
    print(f"y_train: {listOfY_train[i]}\n")
    print(f"X_train (few rows): {listOfX_train[i][0:5,:]}")  # 5 rows
...
for i in range(len(listOfX_train)): 
    X_train_scaled += [scaler.transform(listOfX_train[i])] # scale!
    X_test_scaled += [scaler.transform(listOfX_test[i])] # scale!
    Y_train_scaled += [listOfY_train[i]]  # the predicted/desired labels are not scaled
    Y_test_scaled += [listOfY_test[i]]  # not using the scaler

# reused from above - seeing the scaled data 
ascii_table(X_train_scaled[0][0:5,:],Y_train_scaled[0][0:5],None)

# reused from above - seeing the unscaled data (inverting the scaler)
ascii_table(X_train_scaled[0][0:5,:],Y_train_scaled[0][0:5],scaler)
...
for i in range(len(X_train_scaled)):
    print("\n\n++++++++++  TRAINING:  begin  +++++++++++++++\n\n")
    nn_regressor.fit(X_train_scaled[i], Y_train_scaled[i])
    print("++++++++++  TRAINING:   end  +++++++++++++++")
    print(f"The (squared) prediction error (the loss) for {COLUMNS[i+1]} is {nn_regressor.loss_:<6.3f}")
    print(f"And, its square root:         {nn_regressor.loss_ ** 0.5:<6.3f}")
    print()
...
for i in range(len(X_test_scaled)): 
    ascii_table_for_regressor(X_test_scaled[i],
                          Y_test_scaled[i],
                          nn_regressor,
                          scaler)   # this is our own f'n, above
